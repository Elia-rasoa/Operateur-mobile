...
class MobileSeeder extends Seeder
{
    public function run()
    {
        ...
        $this->db->query(
            ...
            ('0331000001', 'Alice', 15000),
            ('0331000002', 'Bob', 8500),
            ('0372000001', 'Charlie', 22000)"
        );

        if ($this->db->table('transactions')->countAllResults() === 0) {
            $clientIds = $this->getIds('clients', 'telephone');
            $this->db->table('transactions')->insertBatch([
                ['type_op_id' => $typeIds['retrait'], 'client_id' => $clientIds['0331000001'], 'montant' => 500, 'frais_appliques' => 50],
                ['type_op_id' => $typeIds['retrait'], 'client_id' => $clientIds['0331000002'], 'montant' => 1000, 'frais_appliques' => 75],
                ['type_op_id' => $typeIds['transfert'], 'client_id' => $clientIds['0331000001'], 'montant' => 200, 'frais_appliques' => 25],
                ['type_op_id' => $typeIds['retrait'], 'client_id' => $clientIds['0372000001'], 'montant' => 300, 'frais_appliques' => 30],
                ['type_op_id' => $typeIds['depot'], 'client_id' => $clientIds['0331000002'], 'montant' => 5000, 'frais_appliques' => 100],
            ]);
        }
